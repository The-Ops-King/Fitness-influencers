...
class InstagramModule(SourceModule):
    ...
    def run(self, ctx: ModuleContext) -> Iterator[RawRecord]:
        ...
        for profile in self._fetch_profiles(ctx, api, sorted(handles)):
            record = self._build_record(ctx, profile, web)
            if record:
                yield record
    ...
    def _build_record(self, ctx: ModuleContext, profile: dict, web: HttpClient) -> RawRecord | None:
        ...
        # Below the floor cannot be at $5K/mo; above the ceiling is a big name.
        if followers is not None and not (ctx.config.min_followers <= followers <= ctx.config.max_followers):
            return None
